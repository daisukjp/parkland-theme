<?php

    // class booking page

    // all other pages will default to page.php. These pages are useful in that the client cannot change the content on these pages. The content is hard-coded therefore un-editable.

    //Please note that this is a wordpress construct of pages and that other 'pages' on your wordpress site may use a different template.

    //@link https://developer.wordpress.org/themes/basics/template-hierarchy/

    //@package parkland-theme

?>

<?php get_header(); ?>

<div class="taichi-quote-container">
<div>
    <p class="quote">
        “The formless moves to the realm of form; the formed moves back to the realm of formlessness. But this is not something to be reached by striving. The common run of men all alike debate how to reach it. But those who have reached it do not debate, and those who debate have not reached it”.
    </p>
    <p class="chang-tzu" >
        <strong>- Chang Tzu</strong>
    </p>
</div>
</div>

<section class="moveset-section">
    <h1>108 EARLIER HEAVEN MOVESET </h1>
    <div class="accordion-item accordion-active active">
        <div class="accordion-header">
            <h4>Earlier Heaven Moveset 1-36</h4>
        </div>
        <div class="accordion-content">
            <ol>
                <li>Commencement</li>
                <li>Left Grass Bird’s Tail</li>
                <li>Grasp Bird’s Tail</li>
                <li>Single Whip</li>
                <li>Raise Hands &amp; Step Up</li>
                <li>Stork Spreads Wings</li>
                <li>Brush Knee</li>
                <li>Play the Fiddle</li>
                <li>Brush Knee, Twist Step L</li>
                <li>Brush Knee, Twist Step R</li>
                <li>Brush Knee</li>
                <li>Single Whip</li>
                <li>Play the Fiddle</li>
                <li>Chop With Fist</li>
                <li>Step Up, Deflect, Parry, Punch</li>
                <li>Apparent Close Up</li>
                <li>Cross Hands</li>
                <li>Embrace Tiger, Return to Mtn</li>
                <li>Diagonal Single Whip</li>
                <li>Fist Under Elbow</li>
                <li>Step Back, Repulse Monkey R</li>
                <li>Step Back, Repulse Monkey L</li>
                <li>Step Back, Repulse Monkey R</li>
                <li>Slanting Flying</li>
                <li>Raise Hands, Step Up</li>
                <li>Stork Spreads Wings</li>
                <li>Brush Knee Left</li>
                <li>Needle at Sea Bottom</li>
                <li>Fan Through Back</li>
                <li>Turn, Chop with Fist</li>
                <li>Set Up, Deflect, Parry, Punch</li>
                <li>Step Up, Grasp Bird’s Tail</li>
                <li>Single Whip</li>
                <li>Cloud Hands (X5)</li>
                <li>Single Whip</li>
                <li>High Pat On Horse</li>
            </ol>
        </div>
    </div>
    
      <div class="accordion-item">
        <div class="accordion-header">
            <h4>Earlier Heaven Moveset 37-72</h4>
        </div>
        <div class="accordion-content">
          <ol start="37">
            <li>Separation of Right Foot</li>
            <li>Separation of Left Foot</li>
            <li>Turn, Kick</li>
            <li>Brush Knee, Twist Step L</li>
            <li>Brush Knee, Twist Step R</li>
            <li>Step Up, Punch (Hit Floor)</li>
            <li>Turn Chop with Fist</li>
            <li>Step Up, Deflect, Parry, Punch</li>
            <li>Right Foot Kick</li>
            <li>Hit Tiger at Left</li>
            <li>Hit Tiger at Right</li>
            <li>Right Foot Kick</li>
            <li>Strike Ears with Fist</li>
            <li>Left Foot Kick</li>
            <li>Turn, Kick</li>
            <li>Chop with Fist</li>
            <li>Step Up, Deflect, Parry, Punch</li>
            <li>Apparent Close Up</li>
            <li>Cross Hands</li>
            <li>Embrace Tiger, Return to Mtn</li>
            <li>Horizontal Single Whip</li>
            <li>Parting Wild Horse Mane R</li>
            <li>Parting Wild Horse Mane L</li>
            <li>Parting Wild Horse Mane R</li>
            <li>Parting Wild Horse Mane L</li>
            <li>Parting Wild Horse Mane R</li>
            <li>Left Grasp Bird’s Tail</li>
            <li>Single Whip</li>
            <li>Step Up, Grasp Bird’s Tail</li>
            <li>Fair Lady Works Shuttle L</li>
            <li>Fair Lady Works Shuttle R</li>
            <li>Fair lady Works Shuttle L</li>
            <li>Fair Lady Works Shuttle R</li>
            <li>Left Grasp Bird’s Tail</li>
            <li>Step Up, Grasp Bird’s Tail</li>
            <li>Single Whip</li>
        </ol>
        </div>
      </div>
    
    
      <div class="accordion-item">
        <div class="accordion-header"><h4>
            Earlier Heaven Moveset 73-108
        </h4></div>
        <div class="accordion-content">
        <ol start="73">
            <li>Cloud Hands (X7)</li>
            <li>Single Whip</li>
            <li>Snake Creeps Down Low Style</li>
            <li>Golden Cock Stands on Leg R</li>
            <li>Golden Cock Stands on Leg L</li>
            <li>Step Back, Repulse Monkey L</li>
            <li>Step Back, Repulse Monkey R</li>
            <li>Slanting Flying</li>
            <li>Raise Hands, Step Up</li>
            <li>Stork Spreads Wings</li>
            <li>Brush Knee L</li>
            <li>Needle at Sea Bottom</li>
            <li>Fan Through Back</li>
            <li>Turn, Snake Puts Out Tongue</li>
            <li>Step Up, Deflect, Parry, Punch</li>
            <li>Step Up, Grasp Bird’s Tail</li>
            <li>Single Whip</li>
            <li>Cloud Hands (X3)</li>
            <li>Single Whip</li>
            <li>High Pat on Horse</li>
            <li>Cross Hands to Penetrate</li>
            <li>Turn, Cross Legs</li>
            <li>Chop with Fist</li>
            <li>Brush Knee, Punch Pubis</li>
            <li>Step Up, Grasp Bird’s Tail</li>
            <li>Single Whip</li>
            <li>Snake Creeps Down Low Style</li>
            <li>Step up, Form Seven Stars</li>
            <li>Retreat to Right Tiger</li>
            <li>Sweep Lotus with Leg</li>
            <li>Bend Bow, Shoot Tiger</li>
            <li>Chop with Fist</li>
            <li>Step Up, Deflect, Parry,Punch</li>
            <li>Apparent Close Up</li>
            <li>Cross Hands</li>
            <li>Conclusion of Tai Chi</li>
        </ol>
        </div>
      </div>
</section>


<?php get_footer(); ?>